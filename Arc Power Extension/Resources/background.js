...
async function getState() {
  const data = await browser.storage.local.get(["spaces", "currentSpace", "lastActiveByTabId", "updatedAt"]);
  let localState = {
  ...
  const run = async () => {
    switch (message.action) {
      ...
      case "loadTabs": {
        ...
        space.tabs = tabs.map((t) => {
          const existing = tabMap[t.id];
          const base = { id: t.id, title: t.title || "(No title)", url: t.url || "", favIconUrl: t.favIconUrl || "", active: !!t.active };
      ...
      case "addToFavorites": {
        ...
        const space = state.spaces[state.currentSpace];
        if (!space) return null;
        const { url, title, favIconUrl, tabId, pinned } = message;
        if (!url) return null;
